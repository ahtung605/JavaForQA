...
import Lesson08.Online.models.Enemy;
import Lesson08.Online.models.Player;
import Lesson08.Online.models.Tools;
...
//    private int[][] invisibleMap;
    ...
    public GameMap(WindowGame windowGame) {
    ...
    public void launchNewGame() {
        player = new Player("Artur");
        levelCount = 0;
        levelCycle();
        windowGame.refreshInfo(this);
    ...
    private void levelCycle() {
        ...
        spawnEnemy();
        ++levelCount;
        windowGame.recordLog("Start Level " + levelCount);
        windowGame.refreshInfo(this);
        repaint();
    }
...
//                invisibleMap[y][x] = CELL_EMPTY;
        ...
        windowGame.recordLog("Create map. Size " + mapWidth + "x" + mapHeight);
    }
    ...
    private void spawnEnemy() {
        ...
        for (int i = 0; i < enemy.getCountEnemies(); i++) {
            ...
            map[enemyPosY][enemyPosX] = CELL_ENEMY;
...
//        if (invisibleMap[nextY][nextX] == CELL_ENEMY) {
            windowGame.recordLog(player.getName() + " has been attack");
            battle(nextX, nextY);
        }
...
//                invisibleMap[nextY][nextX] = CELL_EMPTY;
    ...
    private boolean isValidPlayerStep(int currentX, int currentY, int nextX, int nextY) {
        if (nextX >= 0 && nextX < mapWidth && nextY >= 0 && nextY < mapHeight) {
            windowGame.recordLog(player.getName() + " move to [" + player.getPosition() + "] success!");
            return true;
        } else {
            player.setPosition(currentX, currentY);
            windowGame.recordLog(player.getName() + " is Invalid! Your move is fail!");
            return false;
        }
    }
    ...
    public void setBackground(Color bg) {
        super.setBackground(Color.DARK_GRAY); // фон карты
    }
